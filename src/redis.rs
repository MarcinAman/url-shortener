...
pub async fn get_redis_service() -> Result<RedisService, RedisError> {
    ...
    let max_attempts: u32 = std::env::var("REDIS_CONNECT_RETRIES")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(30);
    let backoff_ms: u64 = std::env::var("REDIS_CONNECT_BACKOFF_MS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(500);
...
mod tests {
    ...
    async fn test_redis_service_set_then_get() {
        ...
        // Set the value
        let set_result = redis_service
            .set(test_key, test_value, Some(60 * 60 * 24))
            .await;
        ...
        assert_eq!(
            ...
            "Retrieved value doesn't match set value"
        );
    ...
    async fn test_redis_service_get_nonexistent_key() {
        ...
        assert!(
            get_result.is_ok(),
            "Failed to get non-existent key from Redis"
        );
    ...
    async fn test_redis_service_set_nx_prevents_overwrite() {
        ...
        // Set the first value
        let first_set_result = redis_service
            .set(test_key, first_value, Some(60 * 60 * 24))
            .await;
        assert!(
            first_set_result.is_ok(),
            "Failed to set first value in Redis"
        );
        assert!(
            first_set_result.unwrap(),
            "First key should have been set successfully"
        );

        // Verify first value was set
        let first_get_result = redis_service.get(test_key).await;
        assert!(
            first_get_result.is_ok(),
            "Failed to get first value from Redis"
        );
        assert_eq!(first_get_result.unwrap(), Some(first_value.to_string()));

        // Attempt to set the second value (should fail due to NX flag)
        let second_set_result = redis_service
            .set(test_key, second_value, Some(60 * 60 * 24))
            .await;
        assert!(
            second_set_result.is_ok(),
            "Second set operation should succeed but return false"
        );
        assert!(
            !second_set_result.unwrap(),
            "Second set should return false due to NX flag preventing overwrite"
        );

        // Verify the first value is still there (not overwritten)
        let final_get_result = redis_service.get(test_key).await;
        assert!(
            final_get_result.is_ok(),
            "Failed to get final value from Redis"
        );
        assert_eq!(
            final_get_result.unwrap(),
    ...
    async fn test_redis_service_ttl_functionality() {
        ...
        let set_result = redis_service
            .set(test_key, test_value, Some(ttl_seconds))
            .await;
        ...
        assert!(
            immediate_get.is_ok(),
            "Failed to get key immediately after setting"
        );
